...
export default {
  ...
  methods: {
    async getOverlays () {
      ...
      await axios.get('https://backend20220418173746.azurewebsites.net/overlayAPI/api/overlay/all', {
    ...
    async getOverlayDim (selOverlay) {
      let overlayDim = []
      axios.defaults.headers.common.Authorization = localStorage.getItem('token')
      await axios.get('https://backend20220418173746.azurewebsites.net/overlayAPI/api/overlay/dim', { withCredentials: true, params: { overlayName: selOverlay } })
